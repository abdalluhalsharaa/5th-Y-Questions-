...
let dialogState = { onConfirm: null, onCancel: null };

const STORAGE_KEYS = {
  settings: 'medical-app-settings-v8',
  progress: 'medical-app-progress-v8',
  favorites: 'medical-app-favorites-v8',
  wrong: 'medical-app-wrong-v8',
  examState: 'medical-app-exam-state-v8',
  subjectPrefs: 'medical-app-subject-prefs-v8'
};
...
function buildQuestionId(subjectName, sourceType, lectureName, questionNumber, questionText, blockIndex) { return [slugify(subjectName), slugify(sourceType), slugify(lectureName), slugify(questionNumber || String(blockIndex + 1)), hashString(questionText).slice(0, 10)].join('__'); }
...
function startExamSession(questions, mode, direction, sourceLabel, extraMinutes) {
  ...
  currentExam = { mode: mode, direction: direction, sourceLabel: sourceLabel || 'custom', questions: questions.map(function (q) { return Object.assign({}, q); }), currentIndex: 0, answers: new Array(questions.length).fill(null), firstAnswers: new Array(questions.length).fill(null), startTime: Date.now(), totalTime: (questions.length + (extraMinutes || 0)) * 60 * 1000, submitted: false, showAnswer: false, metaVisible: false };
  saveExamState(); showScreen('exam-screen'); renderExam(); if (mode === 'exam') startTimer(); scrollQuestionIntoView(false);
}
  ...
  getEl('question-container').innerHTML = `
    <div class="question-header">
      ...
      <div class="question-actions">
        ...
        <button class="icon-btn" onclick="toggleQuestionLocation()" title="Location">${theme.icons.location}</button>
      </div>
    </div>
    ...
    <div class="options-list">
      ${question.options.map(function (opt, idx) {
        ...
        if (showAnswerState) {
          if (idx === correctIdx) cls += ' correct'; else if (currentExam.answers[currentIndex] === idx && idx !== correctIdx) cls += ' wrong';
        }
        return '<button class="' + cls + '" onclick="selectOption(' + idx + ')">' + escapeHtml(opt) + '</button>';
      }).join('')}
    </div>
    <div id="question-location-box" class="inline-meta ${currentExam.metaVisible ? 'visible' : ''}">
...
function renderGrid() {
  ...
  currentExam.questions.forEach(function (question, idx) {
    ...
    if (idx === currentExam.currentIndex) cls += ' current';
      ...
      if (currentExam.mode === 'training' && currentExam.firstAnswers[idx] !== null) cls += isAnswerCorrect(question, currentExam.firstAnswers[idx]) ? ' answered' : ' wrong';
      else cls += ' answered';
    }
...
function renderExamNav() {
  if (!currentExam) return;
  const nav = getEl('exam-nav'); const idx = currentExam.currentIndex; const last = currentExam.questions.length - 1;
  let left = '<span></span>';
  let right = '<span class="nav-right"></span>';
  if (currentExam.direction === 'twoway' && idx > 0) left = '<button class="btn-secondary" onclick="prevQuestion()">Previous ←</button>';
  if (currentExam.mode === 'training') {
    if (currentExam.showAnswer) right = idx < last ? '<button class="btn-primary nav-right" onclick="nextQuestion()">Next →</button>' : '<button class="btn-primary nav-right" onclick="finishExam()">Finish</button>';
    else if (currentExam.answers[idx] !== null) right = '<button class="btn-small nav-right" onclick="showAnswer()">Show Answer</button>';
  } else if (currentExam.answers[idx] !== null) right = idx < last ? '<button class="btn-primary nav-right" onclick="nextQuestion()">Next →</button>' : '<button class="btn-primary nav-right" onclick="finishExam()">Submit Exam</button>';
...
function selectOption(optionIndex) {
  ...
  const idx = currentExam.currentIndex; const question = currentExam.questions[idx];
  ...
  if (currentExam.mode === 'training') {
    ...
    if (isCorrectNow) {
      ...
      if (wrongQuestions.indexOf(question.id) === -1) { wrongQuestions.push(question.id); saveWrongQuestions(); }
...
function showAnswer() {
  if (!currentExam) return;
  currentExam.showAnswer = true;
  const idx = currentExam.currentIndex; const question = currentExam.questions[idx]; const answer = currentExam.firstAnswers[idx];
  if (answer !== null && !isAnswerCorrect(question, answer) && wrongQuestions.indexOf(question.id) === -1) { wrongQuestions.push(question.id); saveWrongQuestions(); }
  saveExamState(); renderExam();
}
function nextQuestion() { if (!currentExam) return; if (currentExam.currentIndex < currentExam.questions.length - 1) { currentExam.currentIndex += 1; currentExam.showAnswer = false; currentExam.metaVisible = false; saveExamState(); renderExam(); scrollQuestionIntoView(true); } }
function prevQuestion() { if (!currentExam || currentExam.direction !== 'twoway') return; if (currentExam.currentIndex > 0) { currentExam.currentIndex -= 1; if (currentExam.mode === 'training') currentExam.showAnswer = currentExam.answers[currentExam.currentIndex] !== null; currentExam.metaVisible = false; saveExamState(); renderExam(); scrollQuestionIntoView(true); } }
function navigateToQuestion(index) { if (!currentExam) return; if (currentExam.direction === 'oneway' && index !== currentExam.currentIndex) return; currentExam.currentIndex = index; if (currentExam.mode === 'training') currentExam.showAnswer = currentExam.answers[index] !== null; currentExam.metaVisible = false; saveExamState(); renderExam(); scrollQuestionIntoView(true); }
function scrollQuestionIntoView(smooth) { const q = getEl('question-container'); if (!q) return; const top = q.getBoundingClientRect().top + window.scrollY - 12; window.scrollTo({ top: top, behavior: smooth && settings.animations !== false ? 'smooth' : 'auto' }); }
function toggleQuestionLocation() { if (!currentExam) return; currentExam.metaVisible = !currentExam.metaVisible; saveExamState(); const box = getEl('question-location-box'); if (box) box.classList.toggle('visible', currentExam.metaVisible); }
...
function exitExam() { if (currentExam && !currentExam.submitted) askConfirm('هل تريد الخروج؟ سيتم حفظ التقدم الحالي.', function () { saveExamState(); currentExam = null; clearInterval(timerInterval); timerInterval = null; goHome(); }); else { currentExam = null; goHome(); } }
...
function showResults() {
  ...
  if (currentExam.mode === 'exam' && score >= 50) { playCelebrateSound(); if (settings.animations !== false) showFireworks(120, 24); }
...
function populateSearchFilter() { const filter = getEl('search-filter'); if (!filter) return; filter.innerHTML = '<option value="all">All Subjects</option>'; subjects.forEach(function (subject) { filter.innerHTML += '<option value="' + escapeAttribute(subject.id) + '">' + escapeHtml(subject.name) + '</option>'; }); }
...
function clearWrongQuestions() { askConfirm('هل تريد مسح جميع الأسئلة الخاطئة؟', function () { wrongQuestions = []; saveWrongQuestions(); goHome(); showToast('تم مسح قائمة الأسئلة الخاطئة.', 'success'); }); }
function toggleFavorite(questionId) { const idx = favorites.indexOf(questionId); if (idx > -1) favorites.splice(idx, 1); else favorites.push(questionId); saveFavorites(); if (currentExam && !currentExam.submitted) renderExam(); updateStatisticsIfOpen(); }
...
function clearFavorites() { askConfirm('هل تريد مسح جميع المفضلة؟', function () { favorites = []; saveFavorites(); goHome(); showToast('تم مسح قائمة المفضلة.', 'success'); }); }
...
function renderStatistics() {
  const theme = getTheme(); const content = getEl('stats-content'); const ordered = sortSubjects(subjects);
  ...
  const summaryHtml = `
    <div class="stats-summary-grid">
      ...
      <div class="progress-card"><h4>${theme.icons.favorites} المفضلة</h4><p><span>عدد الأسئلة</span><strong>${favorites.length}</strong></p></div>
      <div class="progress-card"><h4>${theme.icons.wrong} الأسئلة الخاطئة</h4><p><span>عدد الأسئلة</span><strong>${wrongQuestions.length}</strong></p></div>
    </div>
  `;
  const subjectCards = ordered.map(function (subject) {
...
function applySettings() { settings = Object.assign({}, DEFAULT_SETTINGS, settings || {}); document.documentElement.setAttribute('data-dark', String(!!settings.darkMode)); document.documentElement.setAttribute('data-theme', settings.theme || 'default'); document.documentElement.setAttribute('data-animations', String(settings.animations !== false)); syncSettingsControls(); applyThemeUI(); applyBackgroundSound(); applyEffectAudioVolumes(); }
function syncSettingsControls() { const entries = [ ['dark-mode-toggle', 'checked', !!settings.darkMode], ['theme-selector', 'value', settings.theme || 'default'], ['exam-theme-selector', 'value', settings.theme || 'default'], ['sound-selector', 'value', settings.bgSound || 'none'], ['exam-sound-selector', 'value', settings.bgSound || 'none'], ['bg-sound-enabled-toggle', 'checked', settings.bgSoundEnabled !== false], ['exam-bg-sound-enabled-toggle', 'checked', settings.bgSoundEnabled !== false], ['volume-control', 'value', settings.volume == null ? 50 : settings.volume], ['exam-volume-control', 'value', settings.volume == null ? 50 : settings.volume], ['feedback-toggle', 'checked', settings.feedbackEnabled !== false], ['exam-feedback-toggle', 'checked', settings.feedbackEnabled !== false], ['animations-toggle', 'checked', settings.animations !== false] ]; entries.forEach(function (entry) { const el = getEl(entry[0]); if (el) el[entry[1]] = entry[2]; }); }
function applyThemeUI() { const theme = getTheme(); setText('nav-icon-exams', theme.icons.exams); setText('nav-icon-wrong', theme.icons.wrong); setText('nav-icon-favorites', theme.icons.favorites); setText('nav-icon-search', theme.icons.search); setText('nav-icon-statistics', theme.icons.statistics); setText('nav-icon-settings', theme.icons.settings); setText('statistics-title', theme.texts.statsTitle); setText('settings-title', theme.texts.settingsTitle); setText('exam-settings-title', theme.texts.examSettingsTitle); setText('btn-exam-settings', theme.texts.examSettingsButton); setText('results-title', theme.icons.results + ' ' + theme.texts.resultsTitle); const trainingIcon = document.querySelector('#btn-training-mode .mode-icon'); const examIcon = document.querySelector('#btn-exam-mode .mode-icon'); const trainingLabel = document.querySelector('#btn-training-mode .mode-label'); const examLabel = document.querySelector('#btn-exam-mode .mode-label'); if (trainingIcon) trainingIcon.textContent = theme.icons.progress; if (examIcon) examIcon.textContent = theme.icons.exams; if (trainingLabel) trainingLabel.textContent = theme.texts.trainingLabel; if (examLabel) examLabel.textContent = theme.texts.examLabel; updateStartButtonIcon(); }
...
function primeAudioUnlock() { function unlock() { if (bodyGestureAudioUnlocked) return; bodyGestureAudioUnlocked = true; applyBackgroundSound(); document.removeEventListener('click', unlock); document.removeEventListener('touchstart', unlock); document.removeEventListener('keydown', unlock); } document.addEventListener('click', unlock, { once: true }); document.addEventListener('touchstart', unlock, { once: true }); document.addEventListener('keydown', unlock, { once: true }); }
async function resolveAssetPath(candidates) { const list = (candidates || []).filter(Boolean).map(function (item) { return encodeURI(item); }); for (let i = 0; i < list.length; i += 1) { const candidate = list[i]; if (assetPathCache[candidate]) return assetPathCache[candidate]; try { const response = await fetch(candidate, { method: 'HEAD' }); if (response.ok) { assetPathCache[candidate] = candidate; return candidate; } } catch (err) {} } return list[0] || ''; }
...
async function applyBackgroundSound() { const audio = getEl('bg-audio'); if (!audio) return; audio.volume = (settings.volume == null ? 50 : settings.volume) / 100; const key = settings.bgSound || 'none'; const sound = BACKGROUND_SOUNDS[key] || BACKGROUND_SOUNDS.none; if (!settings.bgSoundEnabled || key === 'none' || !sound.file) { audio.pause(); return; } const src = await resolveAssetPath([sound.file, 'audio/' + sound.file, 'assets/audio/' + sound.file]); if (audio.dataset.currentSrc !== src) { audio.src = src; audio.dataset.currentSrc = src; audio.load(); } if (bodyGestureAudioUnlocked) audio.play().catch(function () {}); }
...
function showMiniCelebration() { showFireworks(50, 12); }
function showFireworks(durationFrames, explosionCount) { const canvas = getEl('fireworks-canvas'); if (!canvas) return; canvas.classList.remove('hidden'); const ctx = canvas.getContext('2d'); canvas.width = window.innerWidth; canvas.height = window.innerHeight; const themeColors = { default: ['#2563eb', '#38bdf8', '#10b981', '#f59e0b'], desert: ['#f4c95d', '#db9b41', '#9a5b24', '#fff0c9'], space: ['#8b5cf6', '#38bdf8', '#f0abfc', '#ffffff'], pirates: ['#d9a740', '#7d4b16', '#fdf0c2', '#7ec8e3'], castle: ['#d4af37', '#7c3f98', '#f5f1dc', '#9ea7d8'], lab: ['#00bcd4', '#7ef9ff', '#15d3a2', '#ffffff'] }; const colors = themeColors[settings.theme] || themeColors.default; const particles = []; const bursts = [ { x: canvas.width * 0.3, y: canvas.height * 0.35 }, { x: canvas.width * 0.7, y: canvas.height * 0.4 }, { x: canvas.width * 0.5, y: canvas.height * 0.26 } ]; bursts.forEach(function (burst) { for (let i = 0; i < explosionCount; i += 1) { const angle = (Math.PI * 2 * i) / explosionCount; const speed = 2.5 + Math.random() * 4.8; particles.push({ x: burst.x, y: burst.y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, color: colors[Math.floor(Math.random() * colors.length)], size: Math.random() * 3 + 1.5, life: 1 }); } }); let frame = 0; function animate() { ctx.clearRect(0, 0, canvas.width, canvas.height); particles.forEach(function (p) { p.x += p.vx; p.y += p.vy; p.vy += 0.08; p.life -= 0.012; if (p.life > 0) { ctx.globalAlpha = p.life; ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2); ctx.fill(); } }); frame += 1; if (frame < durationFrames) requestAnimationFrame(animate); else { ctx.clearRect(0, 0, canvas.width, canvas.height); canvas.classList.add('hidden'); } } animate(); }
...
function saveProgress() { if (!currentExam) return; const answersToUse = currentExam.mode === 'exam' ? currentExam.answers : currentExam.firstAnswers; currentExam.questions.forEach(function (question, idx) { if (answersToUse[idx] === null) return; addProgressId('subject:' + question.subjectName, question.id); const actual = question.originalSourceType || question.sourceType; if (actual === 'lecture') addProgressId('lecture:' + question.subjectName + '/' + question.lectureName, question.id); if (actual === 'ai') addProgressId('ai:' + question.subjectName + '/' + question.lectureName, question.id); if (question.batchName) addProgressId('year:' + question.subjectName + '/' + question.batchName, question.id); }); localStorage.setItem(STORAGE_KEYS.progress, JSON.stringify(progress)); updateStatisticsIfOpen(); }
...
function checkResumeExam() { const raw = localStorage.getItem(STORAGE_KEYS.examState); if (!raw) return; try { const saved = JSON.parse(raw); if (!saved || saved.submitted || !Array.isArray(saved.questions) || !saved.questions.length) { clearExamState(); return; } askConfirm('يوجد امتحان غير مكتمل. هل تريد المتابعة من حيث توقفت؟', function () { currentExam = saved; showScreen('exam-screen'); renderExam(); if (currentExam.mode === 'exam') startTimer(); }, function () { clearExamState(); }); } catch (err) { clearExamState(); } }
...
function showDialog(options) {
  ...
  getEl('dialog-title').textContent = options.title || 'تنبيه';
  getEl('dialog-body').innerHTML = options.message || '';
  const cancelBtn = getEl('dialog-cancel');
  const confirmBtn = getEl('dialog-confirm');
...
function hideDialog() { getEl('dialog-overlay').classList.add('hidden'); dialogState.onConfirm = null; dialogState.onCancel = null; }
function handleDialogOverlay(event) { if (event.target.id === 'dialog-overlay') hideDialog(); }
function dialogConfirmAction() { const fn = dialogState.onConfirm; hideDialog(); if (typeof fn === 'function') fn(); }
function dialogCancelAction() { const fn = dialogState.onCancel; hideDialog(); if (typeof fn === 'function') fn(); }
function askConfirm(message, onConfirm, onCancel) { showDialog({ title: 'تأكيد', message: message, showCancel: true, confirmText: 'تأكيد', cancelText: 'إلغاء', onConfirm: onConfirm, onCancel: onCancel }); }
...
window.handleDialogOverlay = handleDialogOverlay;
