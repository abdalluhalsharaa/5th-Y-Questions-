...
/* ============================================
   MEDICAL EXAM PRACTICE - COMPLETE JAVASCRIPT
   ============================================ */
...
const STORAGE_KEYS = {
    settings: 'medical-app-settings-v6',
    progress: 'medical-app-progress-v6',
    favorites: 'medical-app-favorites-v6',
    wrong: 'medical-app-wrong-v6',
    examState: 'medical-app-exam-state-v6',
    subjectPrefs: 'medical-app-subject-prefs-v6'
};
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    primeAudioUnlock();
    prepareStaticEffectAudio().then(function () {
        return loadData();
    }).then(function () {
        checkResumeExam();
...
function displayRandomQuote() {
    ...
    const quoteEl = getEl('random-quote');
    if (!quoteEl) return;
...
async function loadData() {
    ...
    if (!discoveredRepo) throw new Error('Cannot detect GitHub repository information.');
    ...
    for (let i = 0; i < subjectDirs.length; i += 1) {
        const subject = await scanSubjectFolder(subjectDirs[i]);
        if (subject) scanned.push(subject);
    }

    subjects = sortSubjects(scanned);
    normalizeSubjectPreferences();
    allQuestions = subjects.reduce(function (acc, subject) {
        return acc.concat(subject.allQuestions);
    }, []);
...
async function scanSubjectFolder(dirItem) {
    ...
    const aiFolder = subjectItems.find(function (item) {
        return item.type === 'dir' && item.name.toLowerCase() === 'ai';
    });
    ...
    for (let i = 0; i < lectureFiles.length; i += 1) {
        const lectureGroup = await buildLectureGroupFromFile(lectureFiles[i], dirItem.name, 'lecture', startCounter);
        if (lectureGroup) {
            startCounter += lectureGroup.questions.length;
            lectures.push(lectureGroup);
        }
    }

    for (let j = 0; j < aiFiles.length; j += 1) {
        const aiGroup = await buildLectureGroupFromFile(aiFiles[j], dirItem.name, 'ai', startCounter);
        if (aiGroup) {
            startCounter += aiGroup.questions.length;
            aiLectures.push(aiGroup);
        }
    }

    const combinedQuestions = lectures.reduce(function (acc, group) { return acc.concat(group.questions); }, []).concat(aiLectures.reduce(function (acc, group) { return acc.concat(group.questions); }, []));
    const years = buildYearsFromQuestions(dirItem.name, combinedQuestions);
...
function buildYearsFromQuestions(subjectName, questionPool) {
    const batches = new Map();
    questionPool.forEach(function (question) {
        const batchName = String(question.batchName || '').trim();
        if (!batchName) return;
        if (!batches.has(batchName)) batches.set(batchName, []);
        const clone = Object.assign({}, question, { sourceType: 'year', originalSourceType: question.sourceType });
        batches.get(batchName).push(clone);
    });

    return Array.from(batches.entries()).sort(function (a, b) {
        return a[0].localeCompare(b[0], 'en', { sensitivity: 'base' });
    }).map(function (entry) {
        return {
            id: slugify(subjectName) + '__year__' + slugify(entry[0]),
            name: entry[0],
            type: 'year',
            subjectName: subjectName,
            questions: entry[1]
...
function parseQuestionFile(rawText, meta) {
    ...
    for (let i = 0; i < blocks.length; i += 1) {
        const item = parseQuestionBlock(blocks[i], meta, fallbackCounter, i);
        if (item) {
            if (!item.number) item.number = String(fallbackCounter);
            questions.push(item);
            fallbackCounter += 1;
        }
    }
...
function splitQuestionBlocks(text) {
    ...
    const fromHashes = text.split(/(?:^|
)\s*###\s*(?=
|$)/g).map(function (part) {
        return part.trim();
    }).filter(Boolean);
    if (fromHashes.length > 1) return fromHashes;
...
{2,}/).map(function (p) { return p.trim(); }).filter(Boolean);
    ...
    for (let i = 0; i < paragraphs.length; i += 1) {
        ...
        const isNew = current.length > 0 && hasCorrect && looksLikeQuestionStart(paragraph);
        if (isNew) {
...
').map(function (line) { return line.trim(); }).filter(Boolean);
        ...
        if (!lines.some(function (line) { return /^Correct\s*Answer\s*:/i.test(line); })) return null;
        ...
        const answerLineIndex = lines.findIndex(function (line) {
            return /^Correct\s*Answer\s*:/i.test(line);
        });
        if (answerLineIndex === -1) return null;

        const beforeAnswer = lines.slice(startIndex, answerLineIndex);
        const firstOptionIndex = beforeAnswer.findIndex(function (line) {
        ...
        options = beforeAnswer.slice(firstOptionIndex).filter(function (line) {
            return /^[A-E][\)\.\-]\s*/i.test(line);
        });
        ...
        return {
            id: buildQuestionId(meta.subjectName, meta.sourceType, meta.lectureName, questionNumber, questionText, blockIndex),
            number: questionNumber,
            text: questionText,
            options: options,
            correctAnswer: correctAnswer,
            correctIndex: resolveCorrectIndex(options, correctAnswer),
        ...
        console.warn('Question parsing error:', error, block);
        return null;
    }
}
...
function resolveCorrectIndex(options, correctAnswer) {
    ...
    for (let i = 0; i < options.length; i += 1) {
        const optionText = normalizeComparisonText(options[i]);
        if (optionText && (optionText.indexOf(answerText) > -1 || answerText.indexOf(optionText) > -1)) return i;
    }
    return -1;
}
...
function loadSubjectPreferences() {
    try {
        subjectPreferences = Object.assign({ order: [], pinned: [] }, JSON.parse(localStorage.getItem(STORAGE_KEYS.subjectPrefs)) || {});
    } catch (e) {
...
function normalizeSubjectPreferences() {
    ...
    ids.forEach(function (id) {
        if (subjectPreferences.order.indexOf(id) === -1) subjectPreferences.order.push(id);
    });
...
function startSubjectLongPress(subjectId) {
    ...
    subjectLongPressTimer = setTimeout(function () {
        openSubjectActions(subjectId);
    }, 420);
}
...
function goHome() {
    ...
    const review = getEl('results-review');
...
function renderSubjects() {
    const container = getEl('subjects-list');
    const empty = getEl('subjects-empty');
    ...
    const visible = subjects.filter(function (subject) {
        return subject.name.toLowerCase().indexOf(searchTerm) > -1;
    });
...
function handleSubjectOpen(subjectId, event) {
    ...
    if (subjectActionsOpenId === subjectId) return;
...
function openSubject(subjectId) {
    ...
    if (currentSubject.years.length > 0) {
        cards.push(buildCategoryCard('years', currentSubject.name + ' Years', theme.icons.years, currentSubject.years.length, countQuestions(currentSubject.years), true));
    }
...
function backFromSelection() {
    if (currentSelectionMeta && currentSelectionMeta.backContext === 'subject' && currentSubject) {
...
function showSelectionScreen(groups, title, meta) {
    ...
    getEl('selection-title').textContent = title;

    const searchContainer = getEl('selection-search-container');
    const searchInput = getEl('selection-search');
    ...
    const list = getEl('selection-list');
    const theme = getTheme();
    list.innerHTML = '';
    ...
    getEl('selection-footer').classList.add('hidden');
    getEl('direction-selection').classList.add('hidden');
    getEl('timer-options').classList.add('hidden');
    getEl('start-section').classList.add('hidden');
...
function filterSelectionList() {
    const searchTerm = String(getEl('selection-search').value || '').toLowerCase().trim();
    document.querySelectorAll('#selection-list .selection-item').forEach(function (item) {
...
function toggleGroupSelection(idx) {
    ...
    document.querySelectorAll('#selection-list .selection-item').forEach(function (item, itemIdx) {
        item.classList.toggle('selected', selectedGroups.indexOf(itemIdx) > -1);
    });
...
function updateSelectionFooter() {
    ...
    const totalQuestions = selectedGroups.reduce(function (sum, idx) {
        return sum + currentGroups[idx].questions.length;
    }, 0);

    if (selectedGroups.length > 0) {
        footer.classList.remove('hidden');
        getEl('selected-count').textContent = totalQuestions + ' questions selected';
        const input = getEl('question-count-input');
        input.max = totalQuestions;
        input.value = totalQuestions;
        getEl('max-questions-label').textContent = '/ ' + totalQuestions;
    } else {
...
function selectMode(mode) {
    ...
    document.querySelectorAll('.btn-direction').forEach(function (btn) { btn.classList.remove('active'); });
    getEl('direction-selection').classList.remove('hidden');
    getEl('timer-options').classList.add('hidden');
    getEl('start-section').classList.add('hidden');
}
...
function selectDirection(direction) {
    ...
    if (selectedMode === 'exam') {
        ...
        getEl('base-time-display').textContent = count + ' min';
        getEl('extra-time-display').textContent = '+0 min';
        getEl('total-time-display').textContent = count + ' min';
        getEl('timer-options').classList.remove('hidden');
        const btn = getEl('btn-add-extra');
        btn.disabled = false;
...
function addExtraTime() {
    ...
    extraTimeAdded = true;
    const count = parseInt(getEl('question-count-input').value, 10) || 0;
    getEl('extra-time-display').textContent = '+5 min';
    getEl('total-time-display').textContent = (count + 5) + ' min';
    const btn = getEl('btn-add-extra');
    btn.disabled = true;
...
function startExamSession(questions, mode, direction, sourceLabel, extraMinutes) {
    ...
    currentExam = {
        ...
        questions: questions.map(function (q) { return Object.assign({}, q); }),
        currentIndex: 0,
...
function startSpecialExam(questions, mode, direction) {
    startExamSession(shuffleArray(questions.slice()), mode, direction || 'twoway', 'special', 5);
}
...
function renderExam() {
    ...
    if (currentExam.mode === 'training') {
        ...
        progressText += ' · ' + (questions.length - currentIndex) + ' left';
    }
    ...
    const showAnswerState = currentExam.mode === 'training' && currentExam.showAnswer;
    const isFav = favorites.indexOf(question.id) > -1;
    ...
    getEl('question-container').innerHTML = `
        ...
        <div class="options-list">
            ${question.options.map(function (opt, idx) {
                let cls = 'option-btn';
                if (currentExam.answers[currentIndex] === idx) cls += ' selected';
                if (showAnswerState) {
                    if (idx === correctIdx) cls += ' correct';
                    else if (currentExam.answers[currentIndex] === idx && idx !== correctIdx) cls += ' wrong';
                }
...
function renderGrid() {
    ...
    currentExam.questions.forEach(function (question, idx) {
        ...
        else if (currentExam.answers[idx] !== null) {
            if (currentExam.mode === 'training' && currentExam.firstAnswers[idx] !== null) {
                cls += isAnswerCorrect(question, currentExam.firstAnswers[idx]) ? ' answered' : ' wrong';
            } else {
                cls += ' answered';
            }
        }
        ...
        btn.onclick = function () { navigateToQuestion(idx); };
...
function selectOption(optionIndex) {
    ...
    if (currentExam.mode === 'training') {
        ...
        if (isCorrectNow) {
            ...
            showToast(themeSuccessMessage(), 'success');
        } else {
            if (wrongQuestions.indexOf(question.id) === -1) {
...
function showAnswer() {
    ...
    const idx = currentExam.currentIndex;
    const question = currentExam.questions[idx];
    const answer = currentExam.firstAnswers[idx];
...
function toggleGrid() {
    const grid = getEl('question-grid');
    const btn = getEl('btn-grid-toggle');
...
function startTimer() {
    clearInterval(timerInterval);
    const timerEl = getEl('exam-timer');
...
function timeUp() {
    if (!currentExam) return;
    const unanswered = currentExam.answers.filter(function (answer) { return answer === null; }).length;
...
function finishExam() {
    ...
    if (currentExam.mode === 'exam') {
        currentExam.questions.forEach(function (question, idx) {
            const ans = currentExam.answers[idx];
            if (ans !== null && !isAnswerCorrect(question, ans) && wrongQuestions.indexOf(question.id) === -1) {
                wrongQuestions.push(question.id);
            }
        });
        saveWrongQuestions();
...
function showWaitingMessages() {
    const waitDiv = getEl('results-waiting');
    const contentDiv = getEl('results-content');
    const reviewDiv = getEl('results-review');
    const messageEl = getEl('waiting-message');
...
function showResults() {
    ...
    const theme = getTheme();
    showScreen('results-screen');
    getEl('results-title').textContent = theme.icons.results + ' ' + theme.texts.resultsTitle;

    const questions = currentExam.questions;
    const answersToCheck = currentExam.mode === 'exam' ? currentExam.answers : currentExam.firstAnswers;
    const total = questions.length;
    const answeredCount = answersToCheck.filter(function (answer) { return answer !== null; }).length;
    const correct = answersToCheck.reduce(function (sum, answer, idx) {
        return sum + (answer !== null && isAnswerCorrect(questions[idx], answer) ? 1 : 0);
    }, 0);
    const unanswered = total - answeredCount;
    const incorrect = answeredCount - correct;
    const score = total > 0 ? Math.round((correct / total) * 100) : 0;
    const timeSpent = currentExam.endTime ? Math.round((currentExam.endTime - currentExam.startTime) / 1000) : 0;
    ...
    if (score > 50) {
        ...
        showToast(themeCelebrateMessage(score), 'success');
    }
...
function themeCelebrateMessage(score) {
    switch (settings.theme) {
...
function reviewExam() {
    ...
    let html = '<h3 class="mt-20" style="text-align:right">' + getTheme().icons.review + ' Review</h3>';

    currentExam.questions.forEach(function (question, idx) {
        const userAnswer = currentExam.answers[idx];
        const correctIdx = getCorrectIndex(question);
        const isCorrectNow = userAnswer === correctIdx;
        html += `
            <div class="question-container mt-10" style="border-inline-start:4px solid ${isCorrectNow ? 'var(--success)' : 'var(--danger)'};">
                <div class="question-header">
                    ...
                    <span style="color:${isCorrectNow ? 'var(--success)' : 'var(--danger)'}; font-weight:900;">${isCorrectNow ? getTheme().icons.success + ' Correct' : getTheme().icons.error + ' Wrong'}</span>
                </div>
                <p class="question-text">${escapeHtml(question.text)}</p>
                <div class="options-list">
                    ${question.options.map(function (opt, optionIdx) {
                        let cls = 'option-btn';
                        if (optionIdx === correctIdx) cls += ' correct';
                        if (optionIdx === userAnswer && optionIdx !== correctIdx) cls += ' wrong';
...
function populateSearchFilter() {
    const filter = getEl('search-filter');
    if (!filter) return;
...
function performSearch() {
    const query = String(getEl('search-input').value || '').toLowerCase().trim();
    const filter = getEl('search-filter').value;
    ...
    const results = allQuestions.filter(function (question) {
        ...
        return text.indexOf(query) > -1 && matchesFilter;
    });
...
function openReadonly(questionId) {
    const question = allQuestions.find(function (item) { return item.id === questionId; });
    if (!question) return;
    const theme = getTheme();
    const correctIdx = getCorrectIndex(question);
    ...
    getEl('readonly-content').innerHTML = `
        <div class="question-header">
            ...
            <div class="question-actions">
                <button class="icon-btn ${favorites.indexOf(question.id) > -1 ? 'active' : ''}" onclick="toggleFavorite('${question.id}'); openReadonly('${question.id}')">💚</button>
                <button class="icon-btn" onclick="showLocation('${escapeJsString(question.subjectName)}','${escapeJsString(question.lectureName)}','${escapeJsString(question.batchName || '')}','${escapeJsString(question.pageNumber || '')}')">${theme.icons.location}</button>
            </div>
        </div>
        <p class="question-text">${escapeHtml(question.text)}</p>
        <div class="options-list">
            ${question.options.map(function (opt, idx) {
                return '<div class="option-btn ' + (idx === correctIdx ? 'correct' : '') + '" style="cursor:default;">' + escapeHtml(opt) + '</div>';
            }).join('')}
        </div>
        <div class="explanation-box visible"><strong>Explanation:</strong> ${escapeHtml(question.explanation || 'No explanation available.')}</div>
        <div class="inline-meta">
...
function openWrongQuestions() {
    const questions = allQuestions.filter(function (question) {
        return wrongQuestions.indexOf(question.id) > -1;
    });
    if (!questions.length) {
        showToast('لا توجد أسئلة خاطئة حتى الآن.', 'info');
        return;
    }

    showScreen('selection-screen');
    getEl('selection-title').textContent = 'Wrong Questions';
    getEl('selection-search-container').classList.add('hidden');
    getEl('selection-footer').classList.add('hidden');
...
function clearWrongQuestions() {
    if (confirm('هل تريد مسح جميع الأسئلة الخاطئة؟')) {
        ...
        saveWrongQuestions();
        goHome();
        showToast('تم مسح قائمة الأسئلة الخاطئة.', 'success');
    }
}
...
function openFavoriteQuestions() {
    const questions = allQuestions.filter(function (question) {
        return favorites.indexOf(question.id) > -1;
    });
    if (!questions.length) {
        showToast('لا توجد أسئلة مفضلة حتى الآن.', 'info');
        return;
    }

    showScreen('selection-screen');
    getEl('selection-title').textContent = 'Favorite Questions';
    getEl('selection-search-container').classList.add('hidden');
    getEl('selection-footer').classList.add('hidden');
    getEl('selection-list').innerHTML = `
        <div class="empty-state">
...
function clearFavorites() {
    if (confirm('هل تريد مسح جميع المفضلة؟')) {
        ...
        saveFavorites();
        goHome();
        showToast('تم مسح قائمة المفضلة.', 'success');
    }
}

/* ===================== STATISTICS ===================== */
function toggleStatistics() {
    const panel = getEl('statistics-panel');
...
function updateStatisticsIfOpen() {
    const panel = getEl('statistics-panel');
    if (panel && panel.classList.contains('visible')) renderStatistics();
}

function renderStatistics() {
    const theme = getTheme();
    const content = getEl('stats-content');
    const answeredQuestions = new Set();
    Object.keys(progress).forEach(function (key) {
        const entry = progress[key] || { questionIds: [] };
        (entry.questionIds || []).forEach(function (id) { answeredQuestions.add(id); });
    });

    const totalQuestions = allQuestions.length;
    const totalLectures = subjects.reduce(function (sum, subject) { return sum + subject.lectures.length; }, 0);
    const totalAi = subjects.reduce(function (sum, subject) { return sum + subject.ai.length; }, 0);
    const totalYears = subjects.reduce(function (sum, subject) { return sum + subject.years.length; }, 0);
    ...
    const subjectCards = subjects.map(function (subject) {
        const entry = progress['subject:' + subject.name] || { questionIds: [] };
...
function resetProgress() {
    ...
    showToast('تمت إعادة ضبط جميع البيانات.', 'success');
    updateStatisticsIfOpen();
}

/* ===================== SETTINGS AND THEME UI ===================== */
function toggleSettings() {
    getEl('settings-panel').classList.toggle('visible');
}

function toggleExamSettings(show) {
    const modal = getEl('exam-settings-modal');
...
function toggleDarkMode() {
    settings.darkMode = !!getEl('dark-mode-toggle').checked;
    saveSettings();
    applySettings();
}

function changeTheme(themeName) {
    settings.theme = THEMES[themeName] ? themeName : 'default';
    saveSettings();
    applySettings();
    renderSubjects();
    if (currentSubject && getEl('subject-sections-screen').classList.contains('active')) openSubject(currentSubject.id);
    if (currentExam && getEl('exam-screen').classList.contains('active')) renderExam();
    if (getEl('results-screen').classList.contains('active') && getEl('results-content').innerHTML.trim()) showResults();
    updateStatisticsIfOpen();
}

function changeSound(soundName) {
    settings.bgSound = BACKGROUND_SOUNDS[soundName] ? soundName : 'none';
    saveSettings();
    applySettings();
}

function toggleBackgroundSoundEnabled() {
    const active = [getEl('bg-sound-enabled-toggle'), getEl('exam-bg-sound-enabled-toggle')].find(function (el) { return el && el === document.activeElement; });
    settings.bgSoundEnabled = active ? !!active.checked : !!getEl('bg-sound-enabled-toggle').checked;
    saveSettings();
    applySettings();
}

function changeVolume(value) {
    settings.volume = clampNumber(parseInt(value, 10), 0, 100, 50);
    saveSettings();
    applySettings();
}

function toggleFeedbackSounds() {
    const active = [getEl('feedback-toggle'), getEl('exam-feedback-toggle')].find(function (el) { return el && el === document.activeElement; });
    settings.feedbackEnabled = active ? !!active.checked : !!getEl('feedback-toggle').checked;
    saveSettings();
    applySettings();
}

function toggleAnimations() {
    settings.animations = !!getEl('animations-toggle').checked;
    saveSettings();
    applySettings();
}
...
function syncSettingsControls() {
    const entries = [
        ...
        ['volume-control', 'value', settings.volume == null ? 50 : settings.volume],
        ['exam-volume-control', 'value', settings.volume == null ? 50 : settings.volume],
...
function applyThemeUI() {
    const theme = getTheme();
    setText('nav-icon-exams', theme.icons.exams);
    setText('nav-icon-wrong', theme.icons.wrong);
    setText('nav-icon-favorites', theme.icons.favorites);
    setText('nav-icon-search', theme.icons.search);
    setText('nav-icon-statistics', theme.icons.statistics);
    setText('nav-icon-settings', theme.icons.settings);
    setText('statistics-title', theme.texts.statsTitle);
    setText('settings-title', theme.texts.settingsTitle);
    setText('exam-settings-title', theme.texts.examSettingsTitle);
    ...
    const trainingIcon = document.querySelector('#btn-training-mode .mode-icon');
    const examIcon = document.querySelector('#btn-exam-mode .mode-icon');
    const trainingLabel = document.querySelector('#btn-training-mode .mode-label');
    const examLabel = document.querySelector('#btn-exam-mode .mode-label');
    if (trainingIcon) trainingIcon.textContent = theme.icons.progress;
    if (examIcon) examIcon.textContent = theme.icons.exams;
    if (trainingLabel) trainingLabel.textContent = theme.texts.trainingLabel;
    if (examLabel) examLabel.textContent = theme.texts.examLabel;

    updateStartButtonIcon();
}

function updateStartButtonIcon() {
    setText('btn-start-exam', getTheme().texts.startExam);
}
...
async function prepareStaticEffectAudio() {
    getEl('right-audio').src = await resolveAssetPath(['right.mp3', 'audio/right.mp3', 'assets/audio/right.mp3']);
    getEl('wrong-audio').src = await resolveAssetPath(['wrong.mp3', 'audio/wrong.mp3', 'assets/audio/wrong.mp3']);
    getEl('celebrate-audio').src = await resolveAssetPath(['celebrate.mp3', 'audio/celebrate.mp3', 'assets/audio/celebrate.mp3']);
    applyEffectAudioVolumes();
}

function applyEffectAudioVolumes() {
    const volume = (settings.volume == null ? 50 : settings.volume) / 100;
    ['right-audio', 'wrong-audio', 'celebrate-audio'].forEach(function (id) {
...
async function applyBackgroundSound() {
    const audio = getEl('bg-audio');
    if (!audio) return;
    audio.volume = (settings.volume == null ? 50 : settings.volume) / 100;

    const key = settings.bgSound || 'none';
    const sound = BACKGROUND_SOUNDS[key] || BACKGROUND_SOUNDS.none;
    if (!settings.bgSoundEnabled || key === 'none' || !sound.file) {
        audio.pause();
        return;
    }
...
function playEffectSound(type) {
    if (!currentExam || currentExam.mode !== 'training') return;
    if (settings.feedbackEnabled === false) return;
    const audio = getEl(type === 'right' ? 'right-audio' : 'wrong-audio');
...
function playCelebrateSound() {
    const audio = getEl('celebrate-audio');
    if (!audio || !audio.src) return;
    try {
        audio.currentTime = 0;
        audio.play().catch(function () {});
...
function showFireworks(durationFrames, explosionCount) {
    const canvas = getEl('fireworks-canvas');
    if (!canvas) return;
    ...
    const themeColors = {
        default: ['#2563eb', '#38bdf8', '#10b981', '#f59e0b'],
        desert: ['#f4c95d', '#db9b41', '#9a5b24', '#fff0c9'],
        space: ['#8b5cf6', '#38bdf8', '#f0abfc', '#ffffff'],
        pirates: ['#d9a740', '#7d4b16', '#fdf0c2', '#7ec8e3'],
        castle: ['#d4af37', '#7c3f98', '#f5f1dc', '#9ea7d8'],
        lab: ['#00bcd4', '#7ef9ff', '#15d3a2', '#ffffff']
    };
    ...
    const bursts = [
        ...
        { x: canvas.width * 0.5, y: canvas.height * 0.26 }
    ];
...
function loadSettings() {
    try {
        settings = Object.assign({}, DEFAULT_SETTINGS, JSON.parse(localStorage.getItem(STORAGE_KEYS.settings)) || {});
    } catch (e) {
...
function addProgressId(key, questionId) {
    ...
    if (progress[key].questionIds.indexOf(questionId) === -1) progress[key].questionIds.push(questionId);
}
...
function showToast(message, type) {
    ...
    const theme = getTheme();
    const prefix = theme.toast[type] || theme.toast.info;
