...
function calculateLectureChecklistStats(subject){ const lectures=subject.lectures || []; const completed=lectures.filter(g=>!!state.checklistCompleted[g.id]).length; const total=lectures.length; return { total, completed, remaining:Math.max(0,total-completed), percentage: total?Math.round((completed/total)*100):0 }; }
...
function getSubjectVisibilitySettings(subjectId){ return Object.assign({ lectures:true, years:true, ai:true }, state.subjectStatsSettings[subjectId] || {}); }
...
function renderSubjectsStatsList(){
  ...
  container.innerHTML = subjectsToShow.map(subject => {
    ...
    const expanded = !!state.statsExpand.subjects[subject.id];
    const subjectSettings = getSubjectVisibilitySettings(subject.id);
    const sections = [];
    if(subject.lectures.length && !isSectionExcluded('lecture') && subjectSettings.lectures !== false){
      const analytics = getSectionAnalytics(subject,'lecture');
      sections.push(renderSectionAnalyticsCard(subject,'lecture','المحاضرات',t.icons.lectures,analytics));
    }
    if(subject.years.length && !isSectionExcluded('year') && subjectSettings.years !== false){
      sections.push(renderSectionAnalyticsCard(subject,'year','السنوات',t.icons.years,getSectionAnalytics(subject,'year')));
    }
    if(subject.ai.length && !isSectionExcluded('ai') && subjectSettings.ai !== false){
      sections.push(renderSectionAnalyticsCard(subject,'ai','الذكاء الاصطناعي',t.icons.ai,getSectionAnalytics(subject,'ai')));
    }
    const detailsHtml = sections.length ? sections.join('') : '<div class="stats-empty-note">لا توجد أقسام مرئية لهذه المادة حالياً.</div>';
    return `
      <div class="stats-subject-card ${expanded?'expanded':''}">
        <div class="stats-subject-head">
          <div class="stats-subject-title-row">
            <div class="stats-subject-title">${t.icons.subject} ${escapeHtml(subject.name)}</div>
            <button class="stats-expand-btn" onclick="openSubjectStats('${subject.id}')">${expanded ? '▴ إخفاء' : '▾ عرض التفاصيل'}</button>
          </div>
        ...
        <div class="stats-subject-details">
          <div class="stats-card-actions"><button class="btn-secondary btn-mini" onclick="openSubjectStatsSettings('${subject.id}')">⚙️ إعدادات المادة</button></div>
          ${detailsHtml}
        </div>
      </div>
    `;
...
function openSubjectStats(subjectId){
  state.statsExpand.subjects[subjectId] = !state.statsExpand.subjects[subjectId];
  renderStatisticsPage();
}
function closeSubjectStats(){ renderStatisticsPage(); }
function renderSubjectStats(){ renderStatisticsPage(); }
function openSubjectCategoryFromStats(){ return; }
function openSubjectStatsSettings(subjectId){
  const subject = state.subjects.find(s => s.id === subjectId);
  if(!subject) return;
  state.statsModalSubjectId = subjectId;
...
function applySubjectStatsSettings(){
  ...
  state.statsExpand.subjects[subject.id] = true;
  renderStatisticsPage();
...
function updateStatisticsIfOpen(){ if(el('statistics-screen') && el('statistics-screen').classList.contains('active')) renderStatisticsPage(); }
...
function subjectMetaRows(subject){
  ...
  if(subject.years.length){ rows.push(`<div><span>Years</span><strong>${subject.years.length} Batches \\ ${countQuestions(subject.years)}Q</strong></div>`); }
...
function renderChecklist(){
  ...
  container.innerHTML = subjects.map(subject => {
    ...
    const expanded = !!state.checklistExpanded[subject.id];
    const lectureRows = subject.lectures.length ? subject.lectures.map(group => {
    ...
    }).join('') : '<div class="stats-empty-note">لا توجد محاضرات ضمن هذه المادة.</div>';
    return `
      <div class="checklist-subject-card ${expanded?'expanded':''}">
        <div class="checklist-subject-head">
          <div class="checklist-subject-title">${escapeHtml(subject.name)}</div>
          <button class="checklist-expand-btn" onclick="toggleChecklistSubject('${subject.id}')">${expanded ? '▴' : '▾'}</button>
        </div>
        <div class="checklist-meta">
          <div><span>إجمالي المحاضرات</span><strong>${checklistStats.total}</strong></div>
          <div><span>المتبقي</span><strong>${checklistStats.remaining}</strong></div>
          <div><span>الإنجاز</span><strong>${checklistStats.percentage}%</strong></div>
        </div>
        <div class="progress-bar"><span style="width:${checklistStats.percentage}%"></span></div>
        <div class="checklist-lectures">${lectureRows}</div>
      </div>
    `;
  }).join('');
}
function toggleChecklistSubject(subjectId){ state.checklistExpanded[subjectId] = !state.checklistExpanded[subjectId]; renderChecklist(); }
function toggleChecklistLecture(groupId){ state.checklistCompleted[groupId] = !state.checklistCompleted[groupId]; saveChecklistStore(); renderChecklist(); updateStatisticsIfOpen(); }
...
function reviewExam(){ if(!state.currentExam) return; const reviewDiv=el('results-review'); reviewDiv.classList.remove('hidden'); let html='<h3 class="mt-20" style="text-align:right">'+theme().icons.review+' Review</h3>'; state.currentExam.questions.forEach((q,idx)=>{ const answersUsed = state.currentExam.mode==='exam' ? state.currentExam.answers : state.currentExam.firstAnswers; const userAnswer=answersUsed[idx]; const correctIdx=getCorrectIndex(q); const ok=userAnswer===correctIdx; html += `<div class="question-container mt-10" style="border-inline-start:4px solid ${ok?'var(--success)':'var(--danger)'};"><div class="question-header"><span class="question-number">Q${escapeHtml(q.number||String(idx+1))}</span><span style="color:${ok?'var(--success)':'var(--danger)'};font-weight:900;">${ok?theme().icons.success+' Correct':theme().icons.error+' Wrong'}</span></div><p class="question-text">${escapeHtml(q.text)}</p><div class="options-list">${q.options.map((opt,i)=>{ let cls='option-btn'; if(i===correctIdx) cls+=' correct'; if(i===userAnswer && i!==correctIdx) cls+=' wrong'; return '<div class="'+cls+'" style="cursor:default;"><span class="option-label">'+LETTERS[i]+')</span>'+escapeHtml(opt)+'</div>'; }).join('')}</div><div class="explanation-box visible"><strong>Explanation:</strong> ${escapeHtml(q.explanation||'No explanation available.')}</div></div>`; }); reviewDiv.innerHTML=html; }
...
'كل صفحة تقرؤها اليوم تبني طبيبًا أفضل غدًا.'
  ...
  try{ await loadData(); }catch(err){ console.error(err); renderSubjects(); renderChecklist(); showToast('تعذر تحميل بيانات GitHub الآن. تحقّق من الاتصال أو بنية المجلدات.','error'); }
  checkResumeExam();
});
