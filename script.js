...
const THEMES = {
 default:{icons:{exams:'📝',wrong:'❌',favorites:'⭐',search:'🔍',statistics:'📊',settings:'⚙️',lectures:'📚',ai:'🤖',years:'📅',start:'🚀',results:'🏆',progress:'🎯',location:'📍',success:'✅',error:'❌',review:'🧾',subject:'📘'},texts:{startExam:'🚀 Start Exam',resultsTitle:'Results',statsTitle:'📊 الإحصائيات',settingsTitle:'⚙️ الإعدادات',examSettingsTitle:'⚙️ Exam Settings',examSettingsButton:'⚙️ Exam Settings',trainingLabel:'Training Mode',examLabel:'Real Exam Mode'}},
 desert:{icons:{exams:'🏹',wrong:'🦂',favorites:'🌵',search:'🔎',statistics:'🧭',settings:'🏕️',lectures:'📜',ai:'🔥',years:'📅',start:'🐪',results:'👑',progress:'🏹',location:'🧭',success:'🤎',error:'🦂',review:'📜',subject:'🏜️'},texts:{startExam:'🐪 Start Journey',resultsTitle:'Majlis Report',statsTitle:'🧭 إحصائيات الرحلة',settingsTitle:'🏕️ إعدادات الخيمة',examSettingsTitle:'🏕️ Exam Camp Settings',examSettingsButton:'🏕️ Exam Settings',trainingLabel:'Training Camp',examLabel:'Journey Exam'}},
 space:{icons:{exams:'🚀',wrong:'☄️',favorites:'🌟',search:'🔭',statistics:'📡',settings:'🤖',lectures:'🛰️',ai:'👽',years:'🪐',start:'🚀',results:'🌌',progress:'🎯',location:'📡',success:'✨',error:'☄️',review:'🧾',subject:'🪐'},texts:{startExam:'🚀 Launch Mission',resultsTitle:'Mission Report',statsTitle:'📡 Mission Analytics',settingsTitle:'🤖 Space Controls',examSettingsTitle:'🤖 Mission Controls',examSettingsButton:'🤖 Mission Settings',trainingLabel:'Training Mission',examLabel:'Space Mission'}},
 pirates:{icons:{exams:'☠️',wrong:'🦈',favorites:'💰',search:'🔎',statistics:'🧭',settings:'⚓',lectures:'🗺️',ai:'🦜',years:'🗓️',start:'☠️',results:'👑',progress:'🏴‍☠️',location:'🧭',success:'🪙',error:'🦈',review:'📜',subject:'⚓'},texts:{startExam:'☠️ Start Voyage',resultsTitle:'Treasure Report',statsTitle:'🧭 Voyage Progress',settingsTitle:'⚓ Captain Settings',examSettingsTitle:'⚓ Voyage Settings',examSettingsButton:'⚓ Voyage Settings',trainingLabel:'Deck Training',examLabel:'Treasure Voyage'}},
 castle:{icons:{exams:'⚔️',wrong:'🐉',favorites:'👑',search:'🔎',statistics:'🛡️',settings:'🏰',lectures:'📜',ai:'🕯️',years:'📅',start:'⚔️',results:'👑',progress:'🏹',location:'🛡️',success:'🛡️',error:'🐉',review:'📜',subject:'🏰'},texts:{startExam:'⚔️ Begin Quest',resultsTitle:'Kingdom Report',statsTitle:'🛡️ Quest Progress',settingsTitle:'🏰 Castle Settings',examSettingsTitle:'🏰 Quest Settings',examSettingsButton:'🏰 Quest Settings',trainingLabel:'Knight Training',examLabel:'Kingdom Trial'}},
 lab:{icons:{exams:'🧪',wrong:'☣️',favorites:'🧬',search:'🔬',statistics:'📈',settings:'⚗️',lectures:'🔬',ai:'🧠',years:'📅',start:'🧪',results:'🏅',progress:'🧫',location:'📍',success:'🧫',error:'☣️',review:'📋',subject:'⚗️'},texts:{startExam:'🧪 Start Experiment',resultsTitle:'Research Report',statsTitle:'📈 Experiment Progress',settingsTitle:'⚗️ Lab Settings',examSettingsTitle:'⚗️ Experiment Settings',examSettingsButton:'⚗️ Experiment Settings',trainingLabel:'Trial Run',examLabel:'Main Experiment'}}
};
...
function aggregateMemorySeries(period, detailed, selectedSubjects){ const entries = Object.entries(state.questionsFirstSeen || {}).map(([qid,info])=>({qid, ts:Number(info.ts||0), subjectName:info.subjectName||'غير معروف'})).filter(x=>x.ts>0).sort((a,b)=>a.ts-b.ts); const now=new Date(); let labels=[]; let buckets=[]; if(period==='weekly'){ for(let i=6;i>=0;i--){ const d=new Date(now); d.setHours(0,0,0,0); d.setDate(now.getDate()-i); const key=d.toISOString().slice(0,10); buckets.push(key); labels.push(d.toLocaleDateString('ar-EG',{weekday:'short', day:'numeric'})); } } else if(period==='monthly'){ for(let i=29;i>=0;i--){ const d=new Date(now); d.setHours(0,0,0,0); d.setDate(now.getDate()-i); const key=d.toISOString().slice(0,10); buckets.push(key); labels.push(d.toLocaleDateString('ar-EG',{month:'numeric', day:'numeric'})); } } else { for(let i=11;i>=0;i--){ const d=new Date(now.getFullYear(), now.getMonth()-i, 1); const key=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0'); buckets.push(key); labels.push(d.toLocaleDateString('ar-EG',{month:'short', year:'2-digit'})); } }
  ...
  if(!detailed){ const data = buckets.map(key=>entries.filter(e=>getBucketKey(e.ts)===key).length); return {labels, series:[{name:'كل المواد', data, color:'#2563eb'}]}; }
  const subjects = selectedSubjects && selectedSubjects.length ? selectedSubjects : Array.from(new Set(entries.map(e=>e.subjectName))).sort(); const palette=['#2563eb','#10b981','#f59e0b','#8b5cf6','#ef4444','#06b6d4','#d946ef','#84cc16','#f97316','#14b8a6']; const series = subjects.map((subject, idx)=>({name:subject, color:palette[idx % palette.length], data:buckets.map(key=>entries.filter(e=>e.subjectName===subject && getBucketKey(e.ts)===key).length)})); return {labels, series}; }
function drawMemoriesChart(canvas, labels, series){ const ctx=canvas.getContext('2d'); const w=canvas.width, h=canvas.height; ctx.clearRect(0,0,w,h); ctx.fillStyle='rgba(255,255,255,0.0)'; ctx.fillRect(0,0,w,h); const left=70, right=20, top=20, bottom=70; const maxVal=Math.max(1, ...series.flatMap(s=>s.data)); ctx.strokeStyle='rgba(148,163,184,.4)'; ctx.lineWidth=1; for(let i=0;i<5;i++){ const y = top + (h-top-bottom)*i/4; ctx.beginPath(); ctx.moveTo(left, y); ctx.lineTo(w-right, y); ctx.stroke(); }
  ...
  let legendX = right+10; const legendY = h-20; ctx.textAlign='left'; series.forEach((s, idx)=>{ const x = left + idx*140; ctx.fillStyle=s.color; ctx.fillRect(x, legendY-10, 18, 4); ctx.fillStyle='#475569'; ctx.fillText(s.name, x+24, legendY-6); }); }
...
function renderMemories(){ if(!el('memories-screen')) return; const subjects = sortSubjects(state.subjects).map(s=>s.name); const filters = el('memory-subject-filters'); const currentSelected = Array.from(document.querySelectorAll('#memory-subject-filters input:checked')).map(x=>x.value); if(filters){ filters.innerHTML = subjects.length ? subjects.map(name=>`<label class="memory-subject-chip"><input type="checkbox" value="${escapeAttribute(name)}" ${(currentSelected.length===0 || currentSelected.includes(name))?'checked':''} onchange="renderMemories()"> <span>${escapeHtml(name)}</span></label>`).join('') : '<div class="stats-empty-note">لا توجد مواد بعد.</div>'; }
